...
- **↻ 새로고침** (헤더) → 연결 상태와 무관하게 모든 서버를 즉시 재조회.
...
| 마커 | 상태 | 의미 |
|---|---|---|
| 🟢 초록 채움 (점멸) | **working** | Claude가 지금 생각/도구 실행/답변 작성 중 |
| 🟠 주황 채움 (점멸) | **waiting** | *당신의* 결정을 기다리는 중 (예: 권한 승인) |
| 🟠 주황 링 (빈 원, 정지) + `stalled` 배지 | **stalled** | 플래그는 "작업 중"인데 몇 시간째 아무 일도 없음 — 아래 설명 |
| ⚪ 회색 | **idle** | 답변 완료, 다음 입력 대기 |

메뉴바 카운트(`✳ 3`)는 **working + waiting**만 셉니다.

### "stalled"에 대하여 (휴리스틱임을 투명하게)

상태 플래그는 Claude Code 자신이 기록하는데, 박제될 수 있습니다: 셸 도구 실행 중 연결이 끊기면(SSH 드롭, 원격 제어 링크 유실 등) claude 프로세스는 멀쩡히 살아서 — 이벤트 루프도 돌고 셸 자식도 유지한 채 — 영영 오지 않을 이벤트를 기다리며, "idle로 복귀" 전환을 끝내 기록하지 못합니다. 밖에서 보면 프로세스 수준의 모든 신호(상태, CPU, TTY, 자식 프로세스)가 진짜 작업 중인 세션과 동일합니다 — 전부 실측으로 확인했습니다. **유일한** 관측 가능한 차이는 트랜스크립트 출력이 멈춘다는 것뿐입니다.

그래서 claude-recents는 플래그가 작업 중인데 **관측 가능한 활동이 3시간 이상 없으면** `stalled`로 표시합니다. 실측 분포상 두 집단은 멀리 떨어져 있습니다 — 진짜 작업은 며칠짜리 자율 루프라도 몇 초~몇 분마다 이벤트를 남기고, 박제된 세션은 며칠씩 침묵합니다. 오판 가능성이 있는 유일한 경우는 출력 없이 3시간 넘게 도는 포그라운드 명령입니다.

stalled 세션은 **토큰을 쓰지 않지만**(API 호출 없음 — 트랜스크립트가 증명) 서버 메모리는 점유합니다. 프로세스를 죽여도 안전합니다: 대화는 디스크에 남고 `claude --resume`으로 이어갈 수 있습니다.
...
로컬은 2초마다 갱신, 원격은 패널이 열려 있으면 10초 / 닫혀 있으면 60초마다 폴링합니다. 서버 연결이 끊기면 마지막으로 받은 데이터를 유지한 채 오류를 구역 안에 표시하고, 데이터가 오래되면 `· 5m old`처럼 나이를 보여줍니다 — 네트워크가 돌아오면 자동으로(또는 새로고침 버튼으로 즉시) 따라잡습니다.
